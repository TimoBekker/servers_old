...
class RepsController extends MainController
{
    ...
    private function _reportUsedres()
    {
        ...
        foreach ($tes as $k => $v) {
            ...
            $arrDataProvider[$k]['sum_params'] = \Yii::$app->db->createCommand(
                'SELECT sum(`count_cores`) as sum_cc, sum(`amount_ram`) as sum_ar, sum(`volume_hdd`) as sum_vh FROM r_equipment WHERE type=:type')
                ->bindValue(':type', $v->id)
                ->queryOne();
    ...
    private function _reportUsedresonvirt()
    {
        ...
        foreach ($parents as $k => $v) {
            ...
            $arrDataProvider[$k]['sum_params'] = \Yii::$app->db->createCommand(
                'SELECT sum(`count_cores`) as sum_cc, sum(`amount_ram`) as sum_ar, sum(`volume_hdd`) as sum_vh FROM r_equipment WHERE parent=:parent')
