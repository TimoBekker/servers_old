                        ...
                        <?php elseif( $tN === SimpleMultiInput::TYPE_INPUT ): ?>
                            <?= Html::textInput(
                                ...
                                (function()use($relationModel,$fN) {
                                    if ( $fN === 'size' ) {
                                        if ( isset($relationModel->$fN) ) {
                                            $ost = (string)(gmp_div_r($relationModel->$fN,'1024'));
                                            if ($ost == 0) {
                                                return (string)(gmp_div($relationModel->$fN,'1024'));
                                            } else {
                                                $fp = (string)(gmp_div($relationModel->$fN,'1000'));
                                                $ost1000 = (string)(gmp_div_r($relationModel->$fN,'1000'));
                                                $sp = $ost1000 / 1000;
                                                return $fp + $sp;
                                            }
                                        } else {
                                ...
                                (function()use($firstIteration, $fN) {
                                    $placeholder = '';
                                    if ( $fN === 'size' ) {
                                        $placeholder = 'Объем в Гб';
                                    }
                                    return [ 'class' => 'form-control input-sm', 'disabled' => $firstIteration ? true : false,
