            ...
            <?php if ( $firstIteration ): ?>
                <div class="row">
                    ...
                    <?php foreach ($fieldsNames as $fieldName): ?>
                        ...
                        <div class="col-md-<?= SimpleMultiInput::genGrid([5,6]) ?>">
                            <?= Html::label(
            ...
            <div class="row multi-field-input <?= $firstIteration ? 'hidden-template-field' : '' ?>" style="<?= $firstIteration ? 'display:none;' : '' ?>">
                ...
                <div class="col-md-0">
                    <?= Html::fileInput(
                        "{$relationModelName}["
                        .(function()use($inputUID,$firstIteration){return $firstIteration ? '{{uid}}' : $inputUID;})()
                        ."][url]", isset($relationModel->url) ? $relationModel->url : null,
                        ['class'=>'input-url','disabled' => $firstIteration ? true : false, 'style'=>"display:none"]
                    ); ?>
                </div>
                <div class="col-md-0">
                    <?= Html::hiddenInput(
                        "{$relationModelName}["
                        .(function()use($inputUID,$firstIteration){return $firstIteration ? '{{uid}}' : $inputUID;})()
                        ."][currentdocid]", isset($relationModel->id) ? $relationModel->id : null,
...
$this->registerJs("
    ...
    $('.".$relationModelName." .multi-field-add').click(function(event) {
        ...
        var addFileInput = $(this).prev().find('.input-url');
        var addNameInput = addFileInput.parent().prev().prev().find('input');
        console.log(addNameInput);
        addFileInput.click();
        addFileInput.one('change', function(event) {
            if (this.files.length > 0) {
                addNameInput.val( this.files[0].name );
            }
        });
    });
", View::POS_READY);
